    ...
    <div class="overflow-x-auto rounded-2xl bg-surface p-2 shadow-sm">
        <div class="min-w-[22rem]">
            ...
            <div class="grid grid-cols-[2.75rem_repeat(5,minmax(0,1fr))] gap-1 border-b border-ink/10 pb-1">
                ...
                >
                    {{ col.label }}
                    <div v-if="col.sublabel" class="text-[11px] font-normal text-ink/30">
                        {{ col.sublabel }}
                    </div>
                </div>
            </div>
            ...
            >
                ...
                >
                    <div
                        v-if="p && p.lunch"
                        class="text-[11px] leading-tight text-ink/80"
                        :title="p.lunch"
                    >
                        🍽 {{ p.lunch }}
                    </div>
                    <div
                        v-if="p && p.activity"
                        class="text-[11px] leading-tight text-hort-purple"
                        :title="p.activity"
                    >
                        🎨 {{ p.activity }}
                    </div>
                    ...
                    <div
                        ...
                        :key="'ex-' + j"
                        class="rounded-md bg-hort-purple/15 px-1 py-0.5 text-[11px] font-semibold leading-tight text-hort-purple"
                        :title="a.name"
                    >
