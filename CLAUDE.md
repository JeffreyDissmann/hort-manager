...
- **Proxying**: thumbnails/downloads stream through the app (`PaperlessController`) so the token stays server-side and the user needs no direct Paperless access; the per-row/card „In Paperless öffnen" is an optional deep link (needs `PAPERLESS_URL` exposed as `paperlessUrl` + the user's own Paperless access). Correspondent names + the document amount are resolved best-effort from Paperless (needs a **superuser** token — `is_staff` isn't enough to read correspondents behind object permissions).
