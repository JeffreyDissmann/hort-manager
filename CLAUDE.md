...
App is German end-to-end (`APP_LOCALE=de`, `lang/de/*` validation/auth messages; `Europe/Berlin` timezone). Built & tested: Kinder+Stammplan, parent↔child roles, Wochenplan/Abholplan timetable, Tagesboard, Ausflug poll, Tagesprogramm + Hausaufgaben, birthdays surfaced on Heute/Abholplan/Programm, role-aware Start page.

**Planned (not built):** Slack integration with the Hort's free-tier Slack (SSO + posting departures/announcements). Richer admin control over users & children (managing which parents belong to which child, user management).
