...
- **Setup** (`/closures`, staff + admins): the period, its Anmeldeschluss, and which days it offers. Creating one generates a `HolidayCareDay` per **weekday** from `Setting::careDefaultWindow()` (08:30–16:00; a `settings` row, no UI to change it yet); editing the range fills gaps and drops days outside it; removing a single day cascades its sign-ups away.
...
- **Sign-up**: parents on **„Ausflüge & Ferien"** (`/polls`) for their own children — one page for everything that wants an answer from a family, so a trip poll and a Ferien sign-up sit under one tab and one badge; staff on `/care` for anyone and also after the deadline (a parent opening `/care` is redirected). Both render `Components/Care/SignupList.vue` from `CareSignupData::for($user)` — `ownChildrenOnly` on the family page, so a staff guardian answers for their own child there and for everyone else on `/care`. Saving sends the full day set for one child and records a `HolidayCareAnswer`.
