...
/**
 * Rejects a Schließzeit that would swallow an Ausflug. A trip can't be booked on a
 * closed day (see {@see NotDuringClosure}) — closing the day afterwards is the same
 * conflict from the other side, and the board would show a closure card while the
 * trip still chases RSVPs. Naming the trip lets staff move or cancel it themselves,
 * which is theirs to decide: silently deleting one would take its answers with it.
 *
